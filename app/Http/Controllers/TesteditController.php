...
class TesteditController extends Controller
{
    ...
    public function editRegister($id, Request $request)
    {
        ...
        // 削除処理
        $write_test_data = $write_test->where('test_id', $id)->delete();
        $select_test_data = $select_test->where('test_id', $id)->delete();
        $hole_test_data = $hole_test->where('test_id', $id)->delete();
        $test_data = $test->where('id', $id)->delete();
